...
```ts
const storesController = new StoresController(client);
```
...
try {
  const { result, ...httpResponse } = await storesController.listStores(brand);
...
const id = '"BOH"';
try {
  const { result, ...httpResponse } = await storesController.getStore(id);
  // Get more response info...
  // const { statusCode, headers } = httpResponse;
} catch(error) {
  if (error instanceof ApiError) {
